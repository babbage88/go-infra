...
func StartWebApiServer(authService *authapi.UserAuthService, userCRUDService *services.UserCRUDService, swaggerSpec []byte, srvadr *string) error {
	...
	mux.Handle("/roles", cors.CORSWithPOST(authapi.AuthMiddlewareRequirePermission(authService, "ReadRole", userapi.GetAllRolesHandler(userCRUDService))))
	mux.Handle("/permissions", cors.CORSWithPOST(authapi.AuthMiddlewareRequirePermission(authService, "ReadPermission", userapi.GetAllAppPermissionsHandler(userCRUDService))))
	mux.Handle("/users", cors.CORSWithPOST(authapi.AuthMiddleware(envars, userapi.GetAllUsersHandler(userCRUDService))))
