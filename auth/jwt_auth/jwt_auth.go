...
func createTokenString(sub string, userInfo interface{}) (string, time.Time, error) {
	var expire_minutes, err = type_helper.ParseInt64(os.Getenv("EXPIRATION_MINUTES"))
	var jwt_algo = os.Getenv("JWT_ALGORITHM")
...
func createToken(userid int32, role string, email string) (services.AuthTokenDao, error) {
...
func CreateTokenanAddToDb(db *sql.DB, userid int32, role string, email string) (services.AuthTokenDao, error) {
