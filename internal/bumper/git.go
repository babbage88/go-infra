...
func FetchTags() error {
	...
	if branch != MasterBranch {
		return fmt.Errorf("error: you must be on the %s branch. Current branch is '%s'", MasterBranch, branch)
	}

	// Step 2: Fetch from remote
	_, err = runGitCommand("fetch", "origin", MasterBranch)
	if err != nil {
	...
	remote, err := runGitCommand("rev-parse", fmt.Sprintf("origin/%s", MasterBranch))
	if err != nil {
		return fmt.Errorf("failed to get remote HEAD: %w", err)
	}

	if local != remote {
		return fmt.Errorf("error: your local %s branch is not up-to-date with remote. Please pull the latest changes", MasterBranch)
	}
