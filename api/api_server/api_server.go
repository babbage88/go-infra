...
func AddApplicationRoutes(mux *http.ServeMux, healthCheckService *user_crud_svc.HealthCheckService, authService authapi.AuthService, roleService *rolesservice.RoleCRUDService, userCRUDService *user_crud_svc.UserCRUDService,
	...
	mux.Handle("POST /roles/permission", cors.CORSWithPOST(authapi.AuthMiddlewareRequirePermission(authService, "AlterRole", userapi.CreateRolePermissionMappingHandler(userCRUDService))))
	mux.Handle("DELETE /roles/permission", cors.CORSWithDELETE(authapi.AuthMiddlewareRequirePermission(authService, "AlterRole", userapi.DeleteRolePermissionMappingHandler(userCRUDService))))
	...
	if sshConnectionManager != nil {
		...
		mux.Handle("GET /ssh/sessions", cors.CORSWithGET(authapi.AuthMiddlewareRequirePermission(authService, "SshConnect", http.HandlerFunc(sshConnectionManager.ListActiveSessionsHandler))))
	}
	...
	mux.Handle("GET /api/v1/proxmox/container", cors.CORSWithGET(
		authapi.AuthMiddlewareRequireRoleOrPermission(authService, "Admin", "ManageHostServers", proxmoxsvc.ListContainersHandler(proxmoxService)),
	))
	mux.Handle("POST /api/v1/proxmox/container/{vmid}/start", cors.CORSWithPOST(
		authapi.AuthMiddlewareRequireRoleOrPermission(authService, "Admin", "ManageHostServers", proxmoxsvc.StartContainerHandler(proxmoxService)),
	))
	...
	mux.Handle("GET /api/v1/proxmox/container/{vmid}/guest-summary", cors.CORSWithGET(
		authapi.AuthMiddlewareRequireRoleOrPermission(authService, "Admin", "ManageHostServers", proxmoxsvc.GetContainerGuestSummaryHandler(proxmoxService)),
	))
	...
	mux.Handle("GET /api/v1/proxmox/workload", cors.CORSWithGET(
		authapi.AuthMiddlewareRequireRoleOrPermission(authService, "Admin", "ManageHostServers", proxmoxsvc.ListWorkloadsHandler(proxmoxService)),
	))
	mux.Handle("GET /api/v1/proxmox/node/options", cors.CORSWithGET(
		authapi.AuthMiddlewareRequireRoleOrPermission(authService, "Admin", "ManageHostServers", proxmoxsvc.GetNodeOptionsHandler(proxmoxService)),
	))
	...
	mux.Handle("GET /api/v1/proxmox/vm/{vmid}/guest-summary", cors.CORSWithGET(
		authapi.AuthMiddlewareRequireRoleOrPermission(authService, "Admin", "ManageHostServers", proxmoxsvc.GetVMGuestSummaryHandler(proxmoxService)),
	))
	...
	mux.Handle("PUT /api/v1/proxmox/vm/{vmid}/hardware/action", cors.CORSWithPUT(
		authapi.AuthMiddlewareRequireRoleOrPermission(authService, "Admin", "ManageHostServers", proxmoxsvc.ApplyVMHardwareActionHandler(proxmoxService)),
	))
